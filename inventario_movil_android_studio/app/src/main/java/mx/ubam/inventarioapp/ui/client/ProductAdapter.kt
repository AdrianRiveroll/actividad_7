...
class ProductAdapter(private val onAdd: (Product) -> Unit) : RecyclerView.Adapter<ProductAdapter.VH>() {
    ...
    inner class VH(private val b: RowProductBinding) : RecyclerView.ViewHolder(b.root) {
        fun bind(p: Product) {
            ...
            b.tvMeta.text = "${p.brand} · Cap: $cap · Stock: ${p.stock} · ${p.price}"
            b.btnAdd.isEnabled = p.stock > 0
            b.btnAdd.setOnClickListener { onAdd(p) }
        }
    }
}
